...
public class PeerConnectionClient {
  ...
  private int DataChannelPhotoSize = 0;
  private int DataChannelPhotoRealSize = 0;
  private int MAXTRANSFER = 65536;
  private int DataChannelFP =0;
  private byte [] DataChannelPhotoFile;
  private String Savepath="";
  
  
//  private VideoRenderer localvr;
//  private VideoRenderer remotevr;
  ...
  public static class PeerConnectionParameters {
    ...
    public Const.Connect_Info connectinfo;

    public PeerConnectionParameters(
        boolean videoCallEnabled, boolean isCaller, boolean loopback, boolean tracing,
        boolean useCamera2, int videoWidth, int videoHeight, int videoFps,
      ...
      Log.d(this.getClass().getName(), String.valueOf(connectinfo.client_size.x));
  ...
  public interface PeerConnectionEvents {
    ...
    /**
     * Callback fired once peer connection send data.
     */
  ...
  private void createPeerConnectionInternal(EglBase.Context renderEGLContext) {
    ...
    if (/*peerConnectionParameters.isCaller && */videoCallEnabled) {
    ...
    pcObserver.setDataChannel(peerConnection.createDataChannel("MeetDay_dataChannel", new DataChannel.Init()));
    pcObserver.showDataChannelLabel();
...
/* TODO: check setEnabled should be enable or renderVideo */
  ...
  public void setDataFileSize(int size) {
	  DataChannelPhotoSize = size;
      DataChannelPhotoRealSize = DataChannelPhotoSize * 3 / 2;
      Log.d(TAG, "photo size = " + DataChannelPhotoSize);
	  DataChannelPhotoFile = new byte[DataChannelPhotoRealSize];
  }
  ...
  public void sendStringByDataChannel(String inputCmd) {
    String cmdString;
    cmdString = "cmdID:" + inputCmd;
    try{
        pcObserver.sendMessage(cmdString);
    } catch(Exception e) {
    	Log.d(TAG, "Send data error!!! Do not send data before ICE connected");
    }
  }
  
  public void sendFilegByDataChannel(String inputCmd, String path) {
	String cmdString;
	int datalength = 0;
	...
	byte[] byteArray = null;
  //isBinaryFile = true;
  //String path = Environment.getExternalStorageDirectory() + peerConnectionParameters.sendText;
	//String path = PictUtil.getSavePath().getAbsolutePath()+"/profile.jpg";//Environment.getExternalStorageDirectory() + "/" + "test" + ".png";
	Log.e(TAG, "+++ path = " + path);
	...
	if (file == null){
		...
		Log.d(TAG, "+++ File is not NULL size = " + file.length());
		//cmdString = "cmdID:" + inputCmd;
		cmdString = inputCmd +":" + file.length();
		pcObserver.sendMessage(cmdString);
		datalength = (int) file.length();
	}
	
	byteArray = FileAccess.convertFileToByteArray(file);
	int fp=0;
	while(datalength>0){
		DataChannel.Buffer buf;
		byte[] b;
		if(datalength > MAXTRANSFER){
			b = new byte[MAXTRANSFER];
			System.arraycopy(byteArray, fp, b, 0, MAXTRANSFER);				
		} else {
			b = new byte[datalength];
			System.arraycopy(byteArray, fp, b, 0, datalength);
		}
		byteBuffer = ByteBuffer.wrap(b);
		buf = new DataChannel.Buffer(byteBuffer, isBinaryFile);		
		fp += MAXTRANSFER;
		datalength -= MAXTRANSFER;
		pcObserver.sendMessage(buf);
	}
	
	//byteBuffer = ByteBuffer.wrap(FileAccess.convertFileToByteArray(file));
	//DataChannel.Buffer buf = new DataChannel.Buffer(byteBuffer, isBinaryFile); 
	//dataChannel.send(buf);
	//pcObserver.sendMessage(buf);
  }

  public void sendGestureByDataChannel(String type, int x, int y) {
    String cmdString;
    cmdString = type + x + "," + y;
//      Log.d(TAG, cmdString);
    try{
  ...
  public void sendModelOrStautsByDataChannel(String type, int gestureType) {
    String cmdString;
    cmdString = type + gestureType;
    try{
        pcObserver.sendMessage(cmdString);
    } catch(Exception e) {
    	Log.d(TAG, "Send data error!!! Do not send data before ICE connected");
  ...
  public void stopVideoSource(final boolean role) {
    executor.execute(new Runnable() {
      @Override
      public void run() {
...
//          if(role)
//        	  localVideoTrack.addRenderer(remotevr);
        }
      }
    });
  }

  public void startVideoSource(final boolean role) {
    executor.execute(new Runnable() {
      @Override
      public void run() {
        if (videoCapturer != null && videoCapturerStopped) {
          ...
          videoCapturerStopped = false;
//          if(role)
//        	  localVideoTrack.addRenderer(localvr);
        }
      }
    });
  }
  ...
  private class PCObserver implements PeerConnection.Observer, DataChannel.Observer {
    ...
    public void setDataChannel(DataChannel tmpDataChannel) {
      if (peerConnection == null || isError) {
        return;
      }
      this.dataChannel = tmpDataChannel;
      this.dataChannel.registerObserver(this);
    }

    public void showDataChannelLabel() {
      if (peerConnection == null || isError) {
        return;
      }
      Log.d(TAG, "data channel label is " + dataChannel.label());
    }
...
//    public void sendMessage(boolean flag){
    public void sendMessage(String cmdString) {
      ...
      buffer = getByteBuffer(cmdString);
      DataChannel.Buffer dataBuffer = new DataChannel.Buffer(buffer, false);
      Log.d(TAG, "Send data " + getString(dataBuffer.data) + " by " + dataChannel.label());
      dataChannel.send(dataBuffer);
    }
    
    public void sendMessage(DataChannel.Buffer dataBuffer) {
        //ByteBuffer buffer = ByteBuffer.allocate(128);
        //buffer = getByteBuffer(cmdString);
        //DataChannel.Buffer dataBuffer = new DataChannel.Buffer(buffer, false);
        Log.d(TAG, "Send dataBuffer " /*+ getString(dataBuffer.data)*/ + " by " + dataChannel.label());
        dataChannel.send(dataBuffer);
      }
...
//      remoteData = getString(buffer.data);
      if(DataChannelPhotoSize > 0){
    	  //DataChannelPhoto = false;
    	  ByteBuffer data = buffer.data;
    	  byte[] bytes = new byte[ data.capacity()];
        data.get(bytes);
        Log.e(TAG, "+++ Received binary file ! :)" + data.capacity());

        if(DataChannelPhotoRealSize >= DataChannelFP + data.capacity()){
    	  System.arraycopy(bytes,0 ,DataChannelPhotoFile,DataChannelFP ,data.capacity());
        }

    	  DataChannelPhotoSize -= MAXTRANSFER;
    	  DataChannelFP += MAXTRANSFER;
    	  if(DataChannelPhotoSize <= 0){
        	  String path = Savepath;//Const.remoteFileTmpPath;//Environment.getExternalStorageDirectory() + "/Pictures/pic1.jpg";
        	  FileAccess.deletefile(path);
        	  Log.d(TAG, "+++ path = " + path);
        	  File file = new File(path); 
        	  if (file == null){
        		  Log.d(TAG, "+++ File is NULL");
            	  ...
            	  FileAccess.convertByteArrayToFile(DataChannelPhotoFile, file);   
                  file = new File(path);
                  while(!file.exists()){
                	  //Log.e("XXXXXXXXXXXXXXXXXXXXXXXXX","sfdsfdgssg");
                	  file = new File(path);
                  }
                  try {
              		Thread.sleep(500);
            	  ...
            	  String msg = "cmdID:ShowFile";
            	  events.onGetMessage(msg);
            	  //message.what = 17;
            	  //mHandler.sendMessage(message);
        	  }
        	  DataChannelPhotoFile = null;
        	  DataChannelPhotoSize = 0;
              DataChannelPhotoRealSize = 0;
        	  DataChannelFP = 0;
        	  Savepath = "";
    	  }
    	  return;
          ...
          remoteData = getString(buffer.data);
    	  events.onGetMessage(remoteData);
//          Log.d(TAG, "Receive message: " + getString(buffer.data));
      }     
    }
    ...
    public void onDataChannel(final DataChannel dc) {
      executor.execute(new Runnable() {
        ...
        public void run() {
          ...
          setDataChannel(dc);
          Log.d(TAG, "Remote data channel label is " + dc.label());
        }
      });
    }
